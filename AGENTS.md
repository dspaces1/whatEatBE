...
- Recipe saves use `GET /api/v1/recipe-saves` (returns saved copies + user-owned recipes; `id` is null when not saved, `is_saved` flags entries), `POST /api/v1/recipe-saves` (source_type: daily_plan_item | recipe | share, returns `recipe_data`), and `DELETE /api/v1/recipe-saves/:id`.
